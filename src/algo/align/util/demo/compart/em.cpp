/* $Id: em.cpp 140909 2008-09-22 18:25:56Z ucko $
* ===========================================================================
*
...
void CElementaryMatching::x_CompartVolume(vector<Uint8>* phits)
{
    ...
    CSeqDB seqdb_cdna    (m_qdb, CSeqDB::eNucleotide);
        ...
        for(; jdx < idx; ++jdx) {
            ...
            if(gc_q < gc_q_min || gc_q >= gc_q_max) {
                ...
                if(kN * (jdx - jdx0) >= min_matches / 2) {
                    ...
                    // preload genomic sequence
                    seqdb_cdna.GetSequence(ii_cdna->m_Oid, &m_CurSeq_cDNA);
                    x_CompartPair(phits, ii_cdna, ii_genomic,
                                  jdx0, jdx, &idx_compacted);
                    seqdb_cdna.RetSequence(&m_CurSeq_cDNA);
                    m_CurSeq_cDNA = 0;
                }
...
void CElementaryMatching::x_LoadRemapData(const string& qdb, const string& sdb)
{
    ...
    {{
        CSeqDB blastdb (qdb, CSeqDB::eNucleotide);
        m_cDNASeqIds.clear();
        for(int oid (0); blastdb.CheckOrFindOID(oid); ++oid) {
...
{
    CRandom rand (GenerateSeed(m_qdb + m_sdb));
    const string base_sfx ( NStr::IntToString(rand.GetRand()));
    m_lbn_q = GetLocalBaseName(m_qdb, base_sfx);
...
void CElementaryMatching::Run(void)
{
    ...
    x_CreateRemapData(m_qdb, eIM_cDNA);

    // create cDNA index using the current filtering table;
    x_CreateIndex(m_qdb, eIM_cDNA, true);
    ...
    x_CreateIndex(m_qdb, eIM_cDNA, false);
